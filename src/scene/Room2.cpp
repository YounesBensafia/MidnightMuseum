...
void Room2::renderStatus(const glm::mat4& view, const glm::mat4& projection, GLuint shaderProgram) {
    if (modelStatus.indexCount > 0) {
        ...
        // Example transform: adjust as needed for your scene
        modelStatusMatrix = glm::translate(modelStatusMatrix, glm::vec3(-3.0f, 3.0f, -37.0f));
        ...
        modelStatusMatrix = glm::scale(modelStatusMatrix, glm::vec3(4.0f, 4.0f, 4.0f));
...
void Room2::renderPyramid(const glm::mat4& view, const glm::mat4& projection, GLuint shaderProgram) {
    if (modelPyramid.indexCount > 0) {
        ...
        PyramidModelMatrix1 = glm::translate(PyramidModelMatrix1, glm::vec3(-8.0f, 1.5f, -36.0f));
        PyramidModelMatrix1 = glm::rotate(PyramidModelMatrix1, glm::radians(-90.0f), glm::vec3(1, 0, 0));
        PyramidModelMatrix1 = glm::scale(PyramidModelMatrix1, glm::vec3(3.0f, 3.0f, 3.0f));
        ...
        PyramidModelMatrix2 = glm::translate(PyramidModelMatrix2, glm::vec3(8.0f, 0.8f, -38.0f));
        PyramidModelMatrix2 = glm::rotate(PyramidModelMatrix2, glm::radians(-90.0f), glm::vec3(1, 0, 0));
        PyramidModelMatrix2 = glm::scale(PyramidModelMatrix2, glm::vec3(2.0f, 2.0f, 2.0f));
...
void Room2::renderSphinx(const glm::mat4& view, const glm::mat4& projection, GLuint shaderProgram) {
    if (modelSphinx.indexCount > 0) {
        ...
        SphinxModelMatrix = glm::translate(SphinxModelMatrix, glm::vec3(0.0f, 0.0f, -30.0f));
        ...
        SphinxModelMatrix = glm::scale(SphinxModelMatrix, glm::vec3(0.6f, 0.6f, 0.6f));
...
void Room2::renderTorso(const mat4& view, const mat4& projection, GLuint shaderProgram) {
    if (torsoModel.indexCount > 0) {
        ...
        TorsoModelMatrix = translate(TorsoModelMatrix, vec3(4.0f, 0.0f, -38.0f));
...
void Room2::init() {
    ...
    buddhaModel = rm.loadFBXModel("assets/models/buddha_triad.glb");
    TutaModel = rm.loadFBXModel("assets/models/the_bust_of_pharaoh_tutankhamun.glb");
        monsterModel = rm.loadFBXModel("assets/models/monster__sekireinel__totem_kings_raid.glb");
        // Load the Amenhotep / Seki statue
        sekiModel = rm.loadFBXModel("assets/models/seated_statue_of_amenhotep_iii.glb");
                torsoModel = rm.loadFBXModel("assets/models/torso_de_tutmosis_iii.glb");
                modelStatus = rm.loadFBXModel("assets/models/egyptian_miniature.glb");
                modelPyramid = rm.loadFBXModel("assets/models/pyramid_figure_decoration.glb");
                modelSphinx = rm.loadFBXModel("assets/models/sandstone_sphinx_statue.glb");
                showcaseModel = rm.loadFBXModel("assets/models/glass_showcase.glb");
                joseModel = rm.loadFBXModel("assets/models/head_of_an_official_the_josephson_head.glb");
    ...
    doorFrameModel = rm.loadFBXModel("assets/models/doorframe.glb");
    doorModel = rm.loadFBXModel("assets/models/door.glb");
    spotlightModel = rm.loadFBXModel("assets/models/lamp.glb");
...
void Room2::renderTuta(const mat4& view, const mat4& projection, GLuint shaderProgram) {
    if (TutaModel.indexCount > 0) {
        ...
        TutaModelMatrix = translate(TutaModelMatrix, vec3(0.0f, 0.0f, -10.0f));
...
void Room2::renderMonster(const mat4& view, const mat4& projection, GLuint shaderProgram) {
    if (monsterModel.indexCount > 0) {
        ...
        MonsterModelMatrix = translate(MonsterModelMatrix, vec3(0.0f, 0.0f, -38.0f));  // In the floor, further back
...
void Room2::renderSeki(const mat4& view, const mat4& projection, GLuint shaderProgram) {
    if (sekiModel.indexCount > 0) {
        ...
        SekiModelMatrix = translate(SekiModelMatrix, vec3(-3.5f, -10.0f, -38.0f)); // Position: left-back, slightly in floor
...
bool Room2::checkWallCollision(const vec3& newPos) {
    // Collision disabled for north wall (far wall)
    float wallMargin = 0.5f;

    // === SECOND ROOM WALLS ONLY ===
    if (newPos.z < -22.0f) {
        // Only east and west wall collision
        if (newPos.x > 12.0f - wallMargin) return true;
        if (newPos.x < -12.0f + wallMargin) return true;

        // South wall with doorway at z = -23 (opening from x = -3 to x = 3)
...
std::vector<ExhibitData> Room2::loadGalleryData() {
    ...
    data.push_back({
        ...
        "assets/models/rosetta_stone.glb",
        vec3(0.0f, 4.0f, -38.0f),        // Position: Center, near back wall
    ...
    data.push_back({
        "Anubis Statue",
        "assets/models/anubis_statue.glb",
...
void Room2::renderShowcase(const mat4& view, const mat4& projection, GLuint shaderProgram) {
    if (showcaseModel.indexCount > 0) {
        ...
        ShowcaseModelMatrix = translate(ShowcaseModelMatrix, vec3(4.0f, 1.0f, -38.0f));
